...
preflight(){
    ...
    if [ "$lsb_dist" =  "ubuntu" ]; then
        if [ "$dist_version" != "20.04" ] && [ "$dist_version" != "19.04" ] && [ "$dist_version" != "18.10" ] && [ "$dist_version" != "18.04" ] && [ "$dist_version" != "16.04" ]; then
            output "Неподдерживаемая версия Ubuntu. Поддерживаются только Ubuntu 20.04, 19.04, 18.10, 18.04, 16.04."
            exit 2
        fi
        ...
        output "Ubuntu: 20.04, 19.04 18.10, 18.04, 16.04"
        output "Debian: 9, 8"
        output "Fedora: 29, 28"
        output "CentOS: 7"
        output "RHEL: 7"
